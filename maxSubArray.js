...
const maxSubArray_Naive = nums => {
    let maxSum = -Infinity;
    for (let i = 0; i < nums.length; i++) {
        for (let j = i; j < nums.length; j++) {
            const sl = nums.slice(i, j + 1);
            const sum = sl.reduce((acc, cur) => acc + cur);
...
console.assert(maxSubArray_DP([-2, 1, -3, 4, -1, 2, 1, -5, 4]) === 6, '[4,-1,2,1] has the largest sum = 6');
console.assert(maxSubArray_DP([1, 2, 5]) === 8, '[1, 2, 5] has the largest sum = 8');
console.assert(maxSubArray_DP([1, 2, -1]) === 3, '[1, 2] has the largest sum = 3');